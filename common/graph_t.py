...
def plot_one(args, data: pd.DataFrame):
    ...
    ax.errorbar(midpoint + args.t_conv, data.loc[:, 'T12_theo'], yerr=np.abs(data.loc[:, 'T12_int_dist']), fmt="o", label=r"$T_{" +f"{args.x_label}" + "-" + f"{args.y_label}" + r"}$")
    ax.errorbar(midpoint - args.t_conv, data.loc[:, 'T21_theo'], yerr=np.abs(data.loc[:, 'T21_int_dist']), fmt="o", label=r"$T_{" +f"{args.y_label}" + "-" + f"{args.x_label}" + r"}$")
    ...
    ax2.errorbar(midpoint + args.t_conv, data.loc[:, 'tau12'], yerr=np.abs(data.loc[:, 'error_tau12']) * 1.96, fmt="o", label=r"$\tau_{" +f"{args.x_label}" + "-" + f"{args.y_label}" + r"}$")
    ax2.errorbar(midpoint - args.t_conv, data.loc[:, 'tau21'], yerr=np.abs(data.loc[:, 'error_tau21']) * 1.96, fmt="o", label=r"$\tau_{" +f"{args.y_label}" + "-" + f"{args.x_label}" + r"}$")
